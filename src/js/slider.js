const getSlider = (components) => {
  const Slider = (container) => {
    const slider = $('<div>').attr('class', 'slider').appendTo(container);
    const track = $('<div>').attr('class', 'track').appendTo(slider);
    const thumb = $('<div>').attr('class', 'thumb').appendTo(slider);
    let _val = 0;
